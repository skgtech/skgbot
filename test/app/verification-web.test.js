...
describe('Verification Web', () => {
  ...
  describe('Error Cases', () => {
    test('Should not allow double invocation of verificaiton operation', async () => {
      const agent = testLib.getAgent();
      const res = await agent.get(`/verify/${memberFix.verification_code}`);

      expect(res.statusCode).toEqual(200);

      const resTwo = await agent.get(`/verify/${memberFix.verification_code}`);

      expect(resTwo.statusCode).toEqual(200);
      expect(resTwo.text).toInclude('Operation Failed');
      const { calls } = sendMock.mock;
      expect(calls.length).toBe(3);
      expect(calls[2][0]).toEqual('You are not allowed to join this server.');
    });
  });
});
