...
/** @const {Array.<string>} ALLOWED_CORE_DB Which core databases can be nuked */
const ALLOWED_CORE_DB = ['skgbot-test', 'skgbot-dev'];
 ...
 */
testDb.recreateDatabase = async (targetDb = 'skgbot-test') => {
  try {
...
testDb._cleanupDb = async (targetDb) => {
  if (!ALLOWED_CORE_DB.includes(targetDb)) {
